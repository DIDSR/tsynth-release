"""Copied and modified from https://github.com/mazurowski-lab/duke-dbt-data.git"""
...
def dcmread_image(
    ...
    if index is not None:
        index = int(len(pixel_array) / 2) if np.isnan(index) else int(index)
        pixel_array = pixel_array[index]

    view_laterality = view[0].upper()
    image_laterality = _get_image_laterality(pixel_array) if index is not None else _get_image_laterality(
        pixel_array[0])
...
def read_labels(
        filepaths_fp, boxes_fp, labels_fp
) -> pd.DataFrame:
    """Read and merge pandas DataFrames with bounding boxes, file paths, and labels"""
    ...
    for df in [df_boxes, df_filepaths, df_labels]:
        if not all([key in df.columns for key in primary_key]):
            raise AssertionError(
                f"Not all primary key columns {primary_key} are present in csv with {df.columns}"
            )

    df_labels = pd.merge(df_filepaths, df_labels, on=primary_key)
    df_labels = pd.merge(df_labels, df_boxes, on=primary_key, how='left')

    df_labels = df_labels.drop('class') if 'class' in df_labels.keys() else df_labels
